...
<mark>【满足两个同方差的情况下可以得到一个特殊的贝叶斯解释，但不意味着只有这种情况下才可以得到】</mark>
...
考查式（3-7）可发现，由估计误差引起的第三项会对样本外 $R^2$ 产生负面影响，即估计误差越大则样本外 $R^2$ 随着在估计过程中使用的数据增多，这一误差项将趋近于0.但是如果我们仅使用非常少的数据进行估计（$\tau$），则较大的估计误差会很容易造成样本外 $R^2$ 为负，哪怕样本内的 $R^2$ 始终为正。【第二项不可能大于1，只有第三项会使 $R^2$ 为负，但**并不确定**二者此消彼长的交互作用】
...
与式（3-7）一样，投资组合的预期收益率也会受到估计误差的负向影响：预期收益率会随着训练集数据量 $\tau$ 而增大。这背后的原因是 $\hat{\mu}$ 中带有噪声的元素绝对值会变大，降低信号的比例，进而降低投资组合的预期收益率。这背后的经济学直觉是：更高的噪声意味着我们将其中的部分仓位用于针对噪声信息（而非真实收益率信号）进行交易。
