...
考查式（3-7）可发现，由估计误差引起的第三项会对样本外 $R^2$ 产生负面影响，即估计误差越大则样本外 $R^2$ 越小，随着在估计过程中使用的数据增多，这一误差项将趋近于0.但是如果我们仅使用非常少的数据进行估计（$\tau$），则较大的估计误差会很容易造成样本外 $R^2$ 为负，哪怕样本内的 $R^2$ 始终为正。【第二项不可能大于1，只有第三项会使 $R^2$ 为负，但**并不确定**二者此消彼长的交互作用】
