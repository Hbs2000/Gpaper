...
与岭回归不同，该解并不会线性依赖于 $y$，且该解不存在解析解，但是部分算法可以求出Lasso的解析解。
...
例如，假设我们有两个高度正相关的协变量，在这种情况下，在非零系数的变量中无论是只包括其中一个协变量或同时包括这两个协变量，对Lasso目标函数中的损失项几乎没有影响。因此，对于Lasso求解来说，非零系数是只包含其中一个协变量还是两个无关紧要。Lasso具体会得到哪种结果可能取决于一些无关因素，比如数据中的噪声。对数据稍作调整就可能会导致Lasso从选择一个变量转变到另一个变量，最好的办法是在模型中同时使用两个变量的均值，以便让二者的噪声相互抵消。这正是岭回归的处理方式。
